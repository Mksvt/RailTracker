    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Створення, редагування та видалення поїздів</CardDescription>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingTrain ? "Оновіть інформацію про поїзд" : "Додайте новий поїзд до системи"}
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <DialogFooter>
                  ...
                  <Button type="submit">{editingTrain ? "Оновити" : "Створити"}</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
      ...
      <CardContent>
        <div className="grid gap-4">
          {trains.map((train) => (
            <div key={train.id} className="border border-border rounded-lg p-4 bg-secondary/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={getTypeColor(train.type)}>{getTypeText(train.type)}</Badge>
                    </div>
                    <p className="text-muted-foreground">{train.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={() => handleEdit(train)}>
                    <Edit className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => handleDelete(train.id)}>
